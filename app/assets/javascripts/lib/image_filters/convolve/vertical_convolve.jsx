(function LibImageFiltersVerticalConvolveModule() {
  const verticalConvolve = (imageData, tmpImageData, options) => {
    ...
    for (let y=0; y<height; y++) {
      for (let x=0; x<width; x++) {
        ...
        for (let i=0; i<wvSide; i++) {
          const iy = Math.min(height-1, Math.max(0, yy + i - halfSide));
          const ix = xx;
  ...
  Lib.ImageFilter.verticalConvolve = verticalConvolve;
})();
